...
def plot_spec_resid_density(wave_grid, resid, mask, labels, ylim, hist=True, kde=True,
    ...
    for i in range(len(resid)):
        ...
        if scatter=='std':
            ...
            scatter_resids.append((np.percentile(resid[i][mask==1.],84)-np.percentile(resid[i][mask==1.],16))/2)
    ...
    for i in range(len(resid)):
        ...
        ax0.annotate('%s= %0.4f %s= %0.4f'%(bias_label, bias_resids[i],
                     ...
                     xy=(0.3, 0.87), xycoords='axes fraction', fontsize=20, bbox=bbox_props)
    
        ax0.plot(wave_grid, np.mean(resid[i], axis=0), color=cb2[3], lw=2) 
        ax0.plot(wave_grid[50:], np.mean(np.abs(resid[i]), axis=0)[:-50], color=cb2[1], lw=2) 
        
    ax0.tick_params(axis='x',
                    bottom=True,
                    labelbottom=True,
                    labelsize=20,width=1,length=10)
    ax0.set_xlabel(r'Wavelength ($\AA$)',fontsize=20)

    cax = fig.add_axes([0.86, 0.15, .015, 0.72])
    cb = plt.colorbar(a, cax=cax)
    cb.set_label(r'Count', size=25)
...
def tsne_domain_analysis2(x_synth, x_obs, x_synthobs,
    ...
    dot_b = ax1.scatter(B_txa[indices], B_tya[indices], marker='o', c='cornflowerblue', alpha=0.2, s=3)
    dot_a = ax1.scatter(A_txa[indices], A_tya[indices], marker='o', c='firebrick', alpha=0.2, s=3)
    leg1 = ax1.legend((dot_a, dot_b), (r'$\mathcal{X}_{synth}$',
                                       x_obs_label),
                      fontsize=22, frameon=True, fancybox=True, markerscale=2.)

    if zsh_synth is not None:
        dot_b = ax2.scatter(Bz_txa[indices], Bz_tya[indices], marker='o', c='cornflowerblue', alpha=0.2, s=3)
        dot_a = ax2.scatter(Az_txa[indices], Az_tya[indices], marker='o', c='coral', alpha=0.2, s=3)
        leg2 = ax2.legend((dot_a, dot_b), (r'$\mathcal{Z}_{synth}$',
                                           r'$\mathcal{Z}_{obs}$'),
                          fontsize=22, frameon=True, fancybox=True, markerscale=2.)

    dot_b = ax4.scatter(B_txb[indices], B_tyb[indices], marker='o', c='cornflowerblue', alpha=0.2, s=3)
    dot_a = ax4.scatter(AB_txb[indices], AB_tyb[indices], marker='o', c='coral', alpha=0.2, s=3)
    leg4 = ax4.legend((dot_a, dot_b), (r'$\mathcal{X}_{synth\rightarrow obs}$',
                                       x_obs_label),
                      fontsize=22, frameon=True, fancybox=True, markerscale=2.)
